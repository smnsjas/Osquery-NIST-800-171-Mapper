function Test-ControlCriteria {
    ...
    try {
        ...
        if ($Control.control_id -eq "3.1.8") {
            ...
            $assessment.Status = Test-AccountLockoutPolicy -SecurityPolicy $SecurityPolicy -Assessment $assessment
        }
        elseif ($Control.control_id -eq "3.5.7") {
            # Password Complexity Policy
            $assessment.Status = Test-PasswordPolicy -SecurityPolicy $SecurityPolicy -Assessment $assessment
        }
        ...
        if ($IncludeEvidence) {
            ...
            if ($Control.control_id -eq "3.1.8" -and $SecurityPolicy) {
                $evidence['AccountLockoutPolicy'] = $SecurityPolicy.AccountLockoutPolicies
            }
            elseif ($Control.control_id -eq "3.5.7" -and $SecurityPolicy) {
...
function Test-AccountLockoutPolicy {
    param($SecurityPolicy, $Assessment)

    if (-not $SecurityPolicy) {
        $Assessment.Findings += "Security policy not available"
        return "NotAssessed"
    }
...
function Test-PasswordPolicy {
    ...
    if ($null -eq $minLength -or $minLength -lt 8) {
